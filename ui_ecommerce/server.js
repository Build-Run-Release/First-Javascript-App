...
app.get('/paystack/verify', async (req, res) => {
    ...
    if (verification && verification.status === true) {
        // Use verified amount from Paystack (in kobo) / 100
        // Or trust the simulation amount if verification fails/is mocked for dev?
        // For security, we should use verification.data.amount

        let productPrice;
        if (verification.data && verification.data.amount) {
             productPrice = verification.data.amount / 100;
        } else {
             // Fallback for simulation if verified without data (unlikely in real world)
        ...
        // If verification fails (or using placeholder keys), check if we are in dev/simulation mode
        if (PAYSTACK_SECRET_KEY.startsWith('sk_test_placeholder')) {
             ...
             db.run('INSERT INTO orders (buyer_id, product_id, amount, service_fee, seller_amount, status, payment_reference, buyer_confirmed, seller_confirmed, escrow_released) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, 0)',
                ...
                (err) => {
                    ...
                    res.send(`Payment Successful (Simulated)! Funds held in Escrow. <a href="/">Go Home</a>`);
                }
            );
        } else {
